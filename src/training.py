...
def run(data_path, f1_criteria):
    ...
    # Ensure we only select the required features based on config
    df = df[features + [label]]  # Select only the features and label columns
    ...
    # Get the value of rf_max_depth from the config
    rf_max_depth = appconfig.get('Hyperparameters', 'rf_max_depth')
    if rf_max_depth == 'None':
        rf_max_depth = None
    else:
        rf_max_depth = appconfig.getint('Hyperparameters', 'rf_max_depth')
